...
This sample demonstrates how to build a **LangGraph-based hosted agent** with:

- **LangGraph ReAct agent** - Uses LangGraph's `create_react_agent` for tool-calling orchestration
...
In [main.py](main.py), the agent is built using [LangGraph](https://langchain-ai.github.io/langgraph/)'s `create_react_agent`, which creates a ReAct-style agent that can reason about when to call tools and how to use their results. The agent uses `AzureChatOpenAI` from LangChain as the underlying LLM and `MemorySaver` for in-memory conversation checkpointing.

### Local Tools Integration

The agent uses a local Python function (`get_available_hotels`) decorated with LangChain's `@tool` decorator. This simulates a hotel availability API and demonstrates how code-based agents can execute custom server-side logic that prompt agents cannot access.
...
The LangGraph agent is served using the [`azure-ai-agentserver-langgraph`](https://pypi.org/project/azure-ai-agentserver-langgraph/) adapter via the `from_langgraph()` function, which wraps the LangGraph agent into a REST API endpoint compatible with the OpenAI Responses protocol.
...
- `azure-ai-agentserver-langgraph` - AgentServer adapter for LangGraph agents
- `azure-identity` - Azure authentication via DefaultAzureCredential
